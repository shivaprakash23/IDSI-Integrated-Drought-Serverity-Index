...
try :
    ...
    arcpy.SelectLayerByAttribute_management ("lyr", "NEW_SELECTION", '"Minimum" <> 99999 AND "Minimum" > 230')
    
    outIDW = Idw("lyr", zField1, cellSize, power)
    outIDW.save(folder+"\\LST"+zField1+year+".tif")

    arcpy.SelectLayerByAttribute_management ("lyr", "NEW_SELECTION", '"Maximum" <> 0 AND "Maximum" < 350')
